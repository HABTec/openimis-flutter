...
class EnrollmentController extends GetxController
  ...
  Future<void> createFamilyOnline() async {
    ...
    final head = {
      ...
      'genderId': gender.value.isNotEmpty ? gender.value[0].toUpperCase() : null,
      'dob': birthdateController.text,
      'head': true,
      'marital': maritalStatus.value.isNotEmpty ? maritalStatus.value[0].toUpperCase() : null,
      'passport': identificationNoController.text,
      'phone': phoneController.text,
      'email': emailController.text,
      'photo': photo.value != null ? {
        'officerId': 6, // TODO: Replace with actual officerId if available
    ...
    final input = {
      ...
      'clientMutationLabel': 'Create family - ${lastNameController.text} ${givenNameController.text} (${chfidController.text})',
      'headInsuree': head,
      'locationId': selectedVillage.value?.id ?? selectedMunicipality.value?.id ?? selectedDistrict.value?.id ?? selectedRegion.value?.id,
  ...
  Future<void> createFamilyWithOfflineSupport() async {
    ...
    if (isOnline.value) {
      ...
      SnackBars.success('Offline', 'Family saved locally and will sync when online.');
    }
  }
